...
def pil_frames_to_video_tensor(frames: Sequence[Image.Image]) -> torch.Tensor:
    """Convert RGB PIL frames to a ``[3, T, H, W]`` tensor in ``[0, 1]``."""
    if len(frames) == 0:
    ...
    for frame in frames:
        ...
        frame_tensors.append(
            torch.from_numpy(arr).permute(2, 0, 1).contiguous())
    return torch.stack(frame_tensors, dim=1)


def _gaussian_kernel_2d(kernel_size: int, sigma: float, channels: int,
                        device: torch.device,
                        dtype: torch.dtype) -> torch.Tensor:
    coords = torch.arange(
        kernel_size, device=device, dtype=dtype) - (kernel_size - 1) / 2.0
...
def video_psnr(pred: torch.Tensor,
               target: torch.Tensor,
               data_range: float = 1.0,
               eps: float = 1e-8) -> float:
    """Average frame PSNR for ``[3, T, H, W]`` videos in ``[0, 1]``."""
...
def video_ssim(pred: torch.Tensor,
               ...
               k1: float = 0.01,
               k2: float = 0.03) -> float:
    """Average frame SSIM for ``[3, T, H, W]`` videos in ``[0, 1]``."""
    if pred.shape != target.shape:
        raise ValueError(f'Shape mismatch: pred={tuple(pred.shape)} '
                         f'target={tuple(target.shape)}')
    ...
    sigma_x2 = F.conv2d(
        pred * pred, kernel, padding=pad, groups=channels) - mu_x2
    sigma_y2 = F.conv2d(
        target * target, kernel, padding=pad, groups=channels) - mu_y2
    sigma_xy = F.conv2d(
        pred * target, kernel, padding=pad, groups=channels) - mu_xy
...
def save_video_frames(frames: Sequence[Image.Image],
                      path: str,
                      fps: int = 8) -> str:
    """Save PIL frames as an MP4 and return the path."""
